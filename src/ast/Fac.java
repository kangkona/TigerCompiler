...
public class Fac
{
  ...
  // // main class: "Factorial"
	 static ast.mainClass.MainClass factorial = new ast.mainClass.MainClass(
		      "Factorial", "a", new ast.stm.Block(new util.Flist<ast.DecOrStm>().addAll(
...
/**class Sum { public static void main(String[] a) {
 *        System.out.println(new Doit().doit(101));}}
 *class Doit {public int doit(int n) {
 *        int sum;
 *        int i;
 *        i = 0;
 *        sum = 0;
 *        while (i<n)
 *          sum = (sum + i)*4;  // here has been modified
 *        return sum;
 *   }}
 */
// main class Sum
   static ast.mainClass.MainClass sum = new ast.mainClass.MainClass(
      "Sum", "a", new ast.stm.Block(new util.Flist<ast.DecOrStm>().addAll(
        new ast.stm.Print(new ast.exp.Call(
  ...
  // class Doit
   static ast.classs.Class doit = new ast.classs.Class("Doit",null,
    ...
    new util.Flist<ast.method.T>().addAll(new ast.method.Method(
      ...
      new ast.stm.Assign("sum",new ast.exp.Times(new ast.exp.Paren(new ast.exp.Add(
      new ast.exp.Id("sum"),new ast.exp.Id("i"))),new ast.exp.Num(4))))),
      new ast.exp.Id("sum"))));
  public static ast.program.Program  prog_sum = new ast.program.Program(sum,
